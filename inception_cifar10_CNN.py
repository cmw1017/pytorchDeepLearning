# hunkim's inception module + cifar10(*3) Accuracy Max = 73%
...
class InceptionA(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.branch3x3dbl_3 = nn.Conv2d(72, 72, kernel_size=3, padding=1)
...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(264, 60, kernel_size=5) #88*3

        self.incept1 = InceptionA(in_channels=30)
        self.incept2 = InceptionA(in_channels=60)

        self.mp = nn.MaxPool2d(2)
        self.fc = nn.Linear(6600, 10)
...
model = Net()
load_model = Path("./model/inception_cifar_CNN")
if load_model.is_file():
    model.load_state_dict(torch.load('./model/inception_cifar_CNN'))
...
for epoch in range(1, 6):
    train(epoch)
    test()
    torch.save(model.state_dict(), "./model/inception_cifar_CNN")
